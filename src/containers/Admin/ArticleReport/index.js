...
const ArticleReport = (props) => {
  ...
  const dealReport =  async (id, status) => {
    const data = await mutate(
      mutations.DealReportArticleMutation,
      {
        id: Number(id),
        status
      }
    )
    const { dealReportArticle: { isSuccess } = {} } = data
  ...
  const articleRender = (item, record) => {
    const handleClick = () => {
      handlers.go('/article/' + item.id)
    }
    return <button onClick={handleClick} className="link-button">{item.title}</button>
  }
  ...
  return (
    <div className={Less['industry-apply']}>
      ...
      <Spin spinning={loading}>
        <Table
          dataSource={admin && admin.articleReport}
          columns={columns}
          rowKey="id"
        />
      </Spin>
